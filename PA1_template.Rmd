...
such as a [Fitbit](http://www.fitbit.com/), [Nike Fuelband](http://www.nike.com/us/en_us/c/nikeplus-fuelband), or [Jawbone Up](https://jawbone.com/up). These type of devices are part of the "quantified self" movement  
...
```{r steps by day, fig.height= 6, fig.width=8.88, fig.show='asses'}
activity1 %>%
ggplot(aes(x = steps))+ 
...
The Mean of the steps equals `r steps_mean`, while the Median equals `r steps_median`
...
```{r Avg daily activity, fig.height=6, fig.width=8.88, warning=FALSE, message=FALSE}
activity2 %>%
...
The Total Number of Missing Values is `r steps_missing`
  
  
2. Devise a strategy for filling in all of the missing values in the dataset.

*I think the best strategy is to replace missing values with the most recent Non-missing value*
...
```{r steps Non-missing, fig.height=6, fig.width=8.88, fig.show='hold'}
activity3 %>%
    group_by(date) %>%
    summarise(steps = sum(steps))%>%
    ggplot(aes(x = steps))+ 
...
Average Total Number of Steps is `r steps_non.missing$mean`, and Median is `r steps_non.missing$median`
...
```{r activity by weektype, fig.height=6, fig.width=10}
activity4 %>%
